...
@Component
class BehaviourAnalysisClient(
    @Value("\${app.behaviour-analysis.host}") val host: String,
    private val webClient: WebClient = WebClient.create(),
) {
    private val logger = LoggerFactory.getLogger(BehaviourAnalysisClient::class.java)
    fun analyzeBehavior(input: BehaviourCheckInput): Mono<StepResult> {
        logger.debug("Behaviour check for customerId: {}", input.customerId)
        return webClient.post()
            .uri("$host/api/v1/credit-card/behaviour-check")
